...
function AJqGrid(param) {
    ...
    $(tableId).jqGrid({
        ...
        loadComplete: function (xhr) {
            currAJqGridRowData = undefined;
            returnInfoHandle(xhr);
            if (xhr.rows) {
                currAJqGridRowData = xhr.rows;
                ...
                if (typeof (param.loadCompleteRows) == "function") {
                    param.loadCompleteRows(currAJqGridRowData);
                }
            }
    ...
    this.reLoadData = function () {
     ...
     this.getFullRowDataById = function (id) {
         if (currAJqGridRowData && currAJqGridRowData.length > 0) {
             for (var i = 0; i < currAJqGridRowData.length; i++) {
                 if (currAJqGridRowData[i].id == id) {
                     return currAJqGridRowData[i];
                 }
             }
         }
    }
