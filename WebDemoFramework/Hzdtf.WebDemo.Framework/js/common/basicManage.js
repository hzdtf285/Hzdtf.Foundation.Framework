...
function BasicManage(param) {
        ...
        if (param.pageDataUrl) {
            ajaxJsonAsync(param.pageDataUrl, QUERY_REQUEST_TYPE, null, function (returnInfo, data) {
                ...
                for (var i = 0; i < data.functions.length; i++) {
                    ...
                    if (param.formUrl && !isLoadForm && (fun.code == ADD_CODE || fun.code == EDIT_CODE || fun.code == SAVE_CODE || fun.code == UPLOAD_CODE)) {
                        isLoadForm = true;
                            ...
                            if (keys && keys.length > 0) {
                                confirm({
                                    ...
                                    confirmCallback: function () {
                                        ajaxJsonAsync(param.batchDeleteUrl, REMOVE_REQUEST_TYPE, keys, function (returnInfo, data) {
                                            showToastr({
                                                text: returnInfo.msg,
                                                type: "success"
                                            });
     ...
     * @param {any} id ID
     */
    var formFill = function (id, disabled) {
        var currRowData = aJqGrid.getFullRowDataById(id);
        var form = new Form("editForm");
