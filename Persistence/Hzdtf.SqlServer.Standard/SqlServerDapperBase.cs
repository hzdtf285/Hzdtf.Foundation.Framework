...
namespace Hzdtf.SqlServer.Standard
{
    ...
    public abstract partial class SqlServerDapperBase<ModelT> : DapperPersistenceBase<ModelT> where ModelT : SimpleInfo
    {
        ...
        protected string[] GetPartPageSql(int pageIndex, int pageSize, string order = "ORDER BY id")
        {
            string field = $"ROW_NUMBER() OVER({order}) AS RowId";
            int[] page = PagingUtil.PageStartEnd(pageIndex, pageSize, 1);

            return new string[] { field, $"WHERE RowId BETWEEN {page[0]} AND {page[1]}" };
        }
        ...
        protected string GetLastInsertIdSql() => "SELECT SCOPE_IDENTITY()";
